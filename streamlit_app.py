...
# Debug: Print current directory and environment path
st.write("Current directory:", current_dir)
env_path = os.path.join(current_dir, '.env')
st.write("Looking for .env file at:", env_path)
st.write("Does .env file exist?", os.path.exists(env_path))

# Try to find .env file
found_dotenv = find_dotenv()
st.write("Found .env file at:", found_dotenv)

# Debug: Print environment variables before loading .env
st.write("Environment variables BEFORE loading .env:")
st.write("OANDA_API_KEY:", "*" * len(os.getenv('OANDA_API_KEY', '')) if os.getenv('OANDA_API_KEY') else "Not found")
st.write("OANDA_ACCOUNT_ID:", "***" if os.getenv('OANDA_ACCOUNT_ID') else "Not found")

# Load .env file
load_dotenv(dotenv_path=env_path, override=True)

# Debug: Print environment variables after loading .env
st.write("Environment variables AFTER loading .env:")
st.write("OANDA_API_KEY:", "*" * len(os.getenv('OANDA_API_KEY', '')) if os.getenv('OANDA_API_KEY') else "Not found")
st.write("OANDA_ACCOUNT_ID:", "***" if os.getenv('OANDA_ACCOUNT_ID') else "Not found")

# Debug: Print raw environment variables
st.write("Raw environment variables:")
st.write("os.environ:", {k: "*" * len(v) if k == 'OANDA_API_KEY' else '***' for k, v in os.environ.items() if 'OANDA' in k})
...
def get_api_credentials():
    ...
    try:
        ...
        if api_key and account_id:
            ...
            st.sidebar.write(f"Account ID: ***")
            st.sidebar.write(f"API Key length: {len(api_key)}")
            
            # Debug: Print actual values
            st.write("Debug - API Key:", "*" * len(api_key))
            st.write("Debug - Account ID:", "***")
        else:
...
if 'data_fetcher' not in st.session_state:
    ...
    st.write("Debug - Creating DataFetcher with:")
    st.write("- API Key:", "*" * len(api_key) if api_key else "Not found")
    st.write("- Account ID:", "***" if account_id else "Not found")
...
if 'oanda' not in st.session_state:
    api_key, account_id = st.session_state.api_credentials
    st.write("Debug - Creating OandaClient with:")
    st.write("- API Key:", "*" * len(api_key) if api_key else "Not found")
    st.write("- Account ID:", "***" if account_id else "Not found")
